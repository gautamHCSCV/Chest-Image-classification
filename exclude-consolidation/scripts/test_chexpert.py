...
class Config():
    ...
    roc_path = f'../loss_acc_roc/roc-{model_name}_chexpert.svg'
    fta_path = f'../roc_pickle_files/fta_{model_name}_chexpert.pkl'
...
class Chexpertdata():
    ...
    def __getitem__(self,idx):
        image,random_label,*all_labels = self.images[idx]
    ...
    def _get_image_labels(self):
        query = False
        for keys in self.class_idx.keys():
            query |= (self.csv_[keys] == 1)
        self.csv_ = self.csv_.loc[query,:]

        images_path = np.array(self.csv_['Path'])
        disease_numpy = np.array(self.csv_.iloc[:,1:]).argmax(axis=1)
        all_labels = np.array(self.csv_.iloc[:,1:])
        all_labels = np.array(list(map(lambda x:[i if x[i] == 1 else -1 for i in range(len(x))],all_labels)))
        self.images = np.concatenate((images_path.reshape(-1,1),disease_numpy.reshape(-1,1),all_labels),axis=1)
