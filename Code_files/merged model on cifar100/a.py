...
class Model_class(object):
    ...
    def __init__(self,py_model,gpu_id=4):
        self.model = py_model
        self.fpr = {}
        self.tpr = {}
        self.auc_ = {}

        self.best_valid_acc = 0.1
...
schedular = optim.lr_scheduler.StepLR(optimizer,step_size=35,gamma=0.1)
n_epochs=125
...
message = {'pretrained':True,
          'classifier_1':'conv2d(512,100,ker=1,stri=1,pad=0)',
          'cuda':5,
          'train':acc_train,
          'test':acc_test,
          'valid':acc_valid,
          'vaidation accuracy' : model.val_acc,
          'train_accuracy' : model.train_acc,
          'validation_loss' : model.val_loss,
          'train_loss' : model.train_loss}

try:
    torch.save({'model':squeezenet.state_dict(),'message':message},'saved_models/merged_cifar100.pth')
    print('saved')
except Exception as e:
    print(e)
    
model.save_model('saved_models/merged_cifar100_best_model.pth', message)
