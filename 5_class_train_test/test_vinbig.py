...
class Vinbigdata():
    ...
    def __init__(self,train_csv,train_path):
        ...
        self.image_labels = {}
        if os.path.exists('test_vinbig.pkl'):
            print('test_vinbig exists')
            with open('test_vinbig.pkl','rb') as f:
    ...
    def _get_image_labels(self):
        for idx, images in enumerate(self.unique_images):
            ...
            if class_disease:
                self.image_labels[f'{images}.dicom'] = self.class_idx[random.choice(class_disease)] 
            if config.return_logs:
                progress(idx+1,len(self.unique_images))
        with open('test_vinbig.pkl','wb') as f:
