...
config = dict(
    saved_path="saved4/efficientnet.pt",
    best_saved_path = "saved4/efficientnet_best.pt",
    lr=0.001, 
    EPOCHS = 30,
    BATCH_SIZE = 32,
    IMAGE_SIZE = 224,
...
while l>len(images):
    l = len(images)
    for path in images.imgs:
        if ord(path[0][path[0].index('6')+8])>ord('F'):
            images.imgs.remove(path)
print(len(images))
train_data,valid_data,test_data = torch.utils.data.dataset.random_split(images,[30000,2000,319])
