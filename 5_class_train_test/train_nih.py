...
class NIH_data():
    ...
    def __init__(self,csv_path,images_path,txt_path,name):
        ...
        for folder in images_folders:
            ...
            # print(f'==> temp_map:{len(temp_map)}, images_map:{len(self.images_map)}')
        ...
        if os.path.exists(f'{self.name}_labels_nih.pkl'):
            with open(f'{self.name}_labels_nih.pkl','rb') as f:
    ...
    def _images_labels(self):
        for idx,image in enumerate(self.images):
            ...
            if valid_labels:
                random_label = random.choice(valid_labels)
                self.image_labels[image] = self.class_idx[random_label]
            if config.return_logs:
                progress(idx+1,len(self.images))
        with open(f'{self.name}_labels_nih.pkl','wb') as f:
            pickle.dump(self.image_labels,f)
        self.images = list(self.image_labels.items())
        
        
def GetDataloader():
    train_data = NIH_data(config.csv_path,config.train_path_nih,config.train_nih,name='train')
    test_data = NIH_data(config.csv_path,config.train_path_nih,config.test_nih,name='test')
