...
class Model_class(object):
    ...
    def save_model(self,directory,message):
        try:
            torch.save({'best_model':self.best_model,'message':message},directory)
            print("model saved")
...
train_data,validate_data,test_data = torch.utils.data.dataset.random_split(images,[120000,15000,6537])

train_loader = torch.utils.data.DataLoader(dataset=train_data,batch_size=16,shuffle=True)
validate_loader = torch.utils.data.DataLoader(dataset = validate_data,batch_size=16,shuffle=True)
...
check = torch.load('../saved_models/merged_nih4.pth')
squeezenet.load_state_dict(check['model'])

squeezenet.classifier[1] = nn.Conv2d(512, 15, kernel_size=(1, 1), stride=(1, 1))
...
try:
    torch.save({'model':squeezenet.state_dict(),'message':'abc'},'../final/pre_our0_model_15.pth')
    print('saved')
except Exception as e:
    print(e)
    
try:
    model.save_model('../final/pre_our0_best_15.pth','abc')
except Exception as e:
    print(e)
